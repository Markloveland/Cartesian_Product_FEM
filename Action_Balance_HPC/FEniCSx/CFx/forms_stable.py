...
def SUPG_weak_form(V1,u1,v1,c_func,K_vol,V2,u2,v2,K_vol_y,fy,c2_func,K2_vol,fx,K2_vol_x,form='weak'):
    ...
    #fy4 + fy9
    K_vol.append(tau*ufl.dot(c1,ufl.grad(u1))*ufl.dot(c1,ufl.grad(v1))*ufl.dx + tau*u1*ufl.nabla_div(c1)*ufl.dot(c1,ufl.grad(v1))*ufl.dx)
    #fy5 + fy10
    K_vol.append(tau*c_func[2]*v1*ufl.dot(c1,ufl.grad(u1))*ufl.dx + tau*c_func[2]*u1*v1*ufl.nabla_div(c1)*ufl.dx)
    #fy6 + fy11
    K_vol.append(tau*c_func[3]*v1*ufl.dot(c1,ufl.grad(u1))*ufl.dx + tau*c_func[3]*u1*v1*ufl.nabla_div(c1)*ufl.dx)
    #fy7
    ...
    if form == 'weak':
        ...
        K2_vol_x = u1*v1*fx[0]*ufl.dx + u1*ufl.dot(ufl.grad(v1),ufl.as_vector((fx[1],fx[2])))*ufl.dx
    if form == 'strong':
        ...
        fx.append(fx3)
        fx.append(fx4)
        K2_vol_x += u1*v1*fx[1]*ufl.dx + u1*ufl.dot(ufl.grad(v1),ufl.as_vector((fx[2],fx[3])))*ufl.dx
    #################################################################
...
def CG_strong_form(domain1,domain2,V1,V2,SUPG='off'):
    ...
    if  SUPG == 'on':
        #adds on stabilizing term and returns stuff     
        tau,K_vol,fy,K_vol_y,tau2,c2,K2_vol,fx,K2_vol_x = SUPG_weak_form(V1,u1,v1,c_func,K_vol,V2,u2,v2,K_vol_y,fy,c2,K2_vol,fx,K_vol_x,form='strong')
